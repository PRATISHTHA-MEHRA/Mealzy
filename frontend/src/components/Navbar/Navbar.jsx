   ...
   <nav className='bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group/nav overflow-x-hidden'>
    ...
    <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
      ...
      <div className='flex-shrink-0 flex items-center space-x-2 group relative md: -translate-x-4 lg:-translate-x-6 ml-0 md:ml-2'>
        ...
        <GiChefToque className='text-3xl md:text-4xl lg:text-5xl text-amber-500 transition-all group-hover:rotate-12 group-hover:text-amber-400 hover:drop-shadow-[0_0_15px] hover:drop-shadow-500 '/>

        <div className='flex flex-col relative ml-2 max-w-[140px] md:max-w-[160px] lg:max-w-none'>
          <NavLink to='/' className='text-2l md:text-xl lg:text-4xl bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent font-monsieur tracking-wider drop-shadow-[0_2px_2px] drop-shadow-black -translate-x-2 truncate md:truncate-none'>
          Mealzy
          </NavLink>
...
<div className='hidden md:flex items-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end'>
  {navLinks.map((link) => (
    ...
    >
      <span className='mr-2 text-sm md:text-[15px] lg:text-base text-amber-500 group-hover:text-amber-300 transition-all'>{link.icon}</span>
  ...
  {/* MOBILE NAVIGATION */}
        <div
          ...
          `}
        >
          <div className='bg-[#2D1B0E] border-t-4 border-amber-900/40 shadow-lg shadow-amber-900/30'>
            <div className='px-4 py-4 space-y-2'>
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center px-4 py-3 text-sm rounded-2xl transition-all border-2 shadow-md shadow-amber-900/10
                     ${isActive
                        ? 'bg-amber-900/25 border-amber-600/50'
                        : 'bg-transparent border-amber-900/30 hover:bg-amber-900/20 hover:border-amber-600/50'
                      }`
                  }
                >
              ...
              <NavLink
                ...
                onClick={() => setIsOpen(false)}
                className={({ isActive }) =>
                  `flex items-center justify-between px-4 py-3 text-sm rounded-2xl transition-all border-2 shadow-md shadow-amber-900/10
                   ${isActive
                      ? 'bg-amber-900/25 border-amber-600/50'
                      : 'border-amber-900/30 hover:bg-amber-900/20 hover:border-amber-600/50'
                    }`
                }
              >
                <div className='flex items-center'>
                  <span className='mr-3 text-amber-500'><FiShoppingCart /></span>
                  <span className='text-amber-100'>Cart</span>
                </div>

                {totalItems > 0 && (
                  <span className='bg-amber-600 text-amber-100 text-xs min-w-6 h-6 px-2 rounded-full inline-flex items-center justify-center'>
                    {totalItems}
                  </span>
                )}
              </NavLink>

              {/* Optional actions placeholders (keep/remove as you like) */}
              {/* ...existing code... */}
            </div>
          </div>
        </div>
...
{showLoginModal && (
  <div className=' fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4'>
    ...
    w-full max-w-[480px] relative border-4 border-amber-700/30 shadow-[0_0_30px] shadow-amber-500/30'>
      ...
      <h2 className='text-2xl font-bold bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent mb-4 text-center'>Mealzy</h2>
    </div>
  </div>
)}
</nav>
